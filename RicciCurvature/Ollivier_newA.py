...
def _get_single_node_neighbors_distributions(node, direction="successors"):
    ...
    for nbr in neighbors:
        if direction == "predecessors":
            ...
            # print(f'w = {-_Gk.weight(node, nbr)}, W = {np.math.pow(_base, np.math.pow( -_Gk.weight(node, nbr), _exp_power))}')
            # w = np.math.pow(_base, np.math.pow( -_Gk.weight(node, nbr), _exp_power))
            try:
    ...
    # *********** revised ***************
    node_value = _nodes[0][node]
    if node_value.item() == 0:
        alpha1 = 0
    else:
        alpha1 = torch.tanh(node_value)
        alpha1 = alpha1.item() ** 2
        
    # print(f'the node value is {node_value}, the new alpha is {alpha1}\n')

    if nbr_edge_weight_sum > EPSILON:
        # Sum need to be not too small to prevent divided by zero
        distributions = [(1.0 - alpha1) * w / nbr_edge_weight_sum for w, _ in heap_weight_node_pair]
    else:
        ...
        distributions = [(1.0 - alpha1) / len(heap_weight_node_pair)] * len(heap_weight_node_pair)

    nbr = [x[1] for x in heap_weight_node_pair]
    return distributions + [alpha1], nbr + [node]
...
def _distribute_densities(source, target):
    ...
    if _shortest_path == "pairwise":
        ...
        norm = np.ones((pairs[0].shape[0], pairs[1].shape[1]), dtype=np.float32)*3
        
        i = np.where(pairs[0] == source)[0][0]
        j = np.where(pairs[1] == target)[0][0]
        
        norm[i,:] -= 1
        norm[:,j] -= 1
                
        d = _apsp[np.ix_(source_topknbr, target_topknbr)]/norm  # transportation matrix
...
def _compute_ricci_curvature_edges(G: nx.Graph, weight="weight", edge_list=[],
                                   ...
                                   shortest_path="all_pairs", nbr_topk=3000, nodes_ori = None):
...
class OllivierRicci:
    ...
    def compute_ricci_curvature(self, nodes_v):
        """Compute Ricci curvature of edges and nodes.
        The node Ricci curvature is defined as the average of node's adjacency edges.
        ...
        self.G = _compute_ricci_curvature(G=self.G, weight=self.weight,
                                          ...
                                          nbr_topk=self.nbr_topk, nodes_ori = nodes_v)
        
        # print(f'_new_alpha is {new_a}, {len(new_a)}\n')
    ...
    def recal_qexp(self, q, node_v, c = 1, weight="weight"):
        q_exp = q_exponential(q)
        
        for (v1, v2) in self.G.edges():
            w = self.G[v1][v2][weight]
            # self.G[v1][v2][weight] = 1.0/abs(q_exp.q_exponential_series(w))
            
            if (node_v[0][v2].cpu().item() <= 0):
                w1 = -w * c
                self.G[v1][v2][weight] = 1.0/abs(q_exp.q_exponential_series(w1))
            else:
                w1 = w * c
                self.G[v1][v2][weight] = 1.0/abs(q_exp.q_exponential_series(w1))
            
            # print(f'The original w is {w}, after scale {w1}, the new weight is {self.G[v1][v2][weight]}')
