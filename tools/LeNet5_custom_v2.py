...
class LeNet_custom_v2(nn.Module):
    ...
    # network structure
    def __init__(self, model_info, edge_set, device, input_channel = 1):
    ...
    def forward(self, x):
        ...
        # second CNN
        x_cov2 = self.activation(self.CNN(x_pool1, self.conv2.weight, self.conv2.bias.unsqueeze(1), 3, 4))
        ...
        fc1 = self.activation(self.linear(fc, self.fc1, 5, 6))
        
        fc2 = self.activation(self.linear(fc1, self.fc2, 6, 7))
        
        y = self.linear(fc2, self.fc3, 7, 8)
    ...
    def edge_w_batch(self, x):
        ...
        edge_v = (self.CNN_edges(x_tmp, k2, 3, 4)).cpu().detach()
        edge_value = edge_v if edge_value == None else torch.cat((edge_value, edge_v), axis=1)

        x = self.activation(self.CNN(x, self.conv2.weight, self.conv2.bias.unsqueeze(1), 3, 4))
        x_tmp = x
        nodes = torch.cat((nodes, x.view(-1, self.num_flat_features(x))), axis = 1)
        ...
        # fc1
        edge_v = (self.fc_edges(x_tmp, self.fc1, 5, 6)).cpu().detach()
        edge_value = edge_v if edge_value == None else torch.cat((edge_value, edge_v), axis=1)

        x = self.activation(self.linear(x, self.fc1, 5, 6))
        x_tmp = x
        
        nodes = torch.cat((nodes, x), axis = 1)
        
        # fc2    
        edge_v = (self.fc_edges(x_tmp, self.fc2, 6, 7)).cpu().detach()
        edge_value = edge_v if edge_value == None else torch.cat((edge_value, edge_v), axis=1)

        x = self.activation(self.linear(x, self.fc2, 6, 7))
        x_tmp = x
        
        nodes = torch.cat((nodes, x), axis = 1)
        
        # fc3
        edge_v = (self.fc_edges(x_tmp, self.fc3, 7, 8)).cpu().detach()
        edge_value = edge_v if edge_value == None else torch.cat((edge_value, edge_v), axis=1)
