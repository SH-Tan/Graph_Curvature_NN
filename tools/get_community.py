...
def analyze_graph_structure_with_community_indegree(curvature, b, prefix_dims, node, threshold=0.0):
    full_edges = set()
    neg_edges = set()
    threshold_edges = set()
    ...
    output_layer_nodes = set(range(prefix_dims[-2], prefix_dims[-1]))
    threshold_nodes = set()
    neg_outgoing = defaultdict(set)
    for u, v in threshold_edges:
    ...
    for root in candidate_roots:
        ...
        while queue:
            ...
            if node_ in visited:
                continue
            visited.add(node_)
            community_nodes.add(node_)
            node_communities[node_].add(community_id)
            for prev in neg_incoming.get(node_, []):
                community_edge_set.add((prev, node_))
                if prev not in visited:
                    queue.append(prev)

        if community_nodes:
            total_internal_curvature = sum(edge_curvatures.get((u, v), 0.0) for (u, v) in community_edge_set)
            # --- Whole community stats ---
            total_nodes_all = len(community_nodes)
            total_edges_all = len(community_edge_set)
            curvature_all = total_internal_curvature

            # --- Without input layer ---
            nodes_wo_input = {n for n in community_nodes if n not in input_layer_nodes}
            edges_wo_input = {(u, v) for (u, v) in community_edge_set if u not in input_layer_nodes and v not in input_layer_nodes}
            curvature_wo_input = sum(edge_curvatures.get((u, v), 0.0) for (u, v) in edges_wo_input)

            # --- Without output layer ---
            nodes_wo_output = {n for n in community_nodes if n not in output_layer_nodes}
            edges_wo_output = {(u, v) for (u, v) in community_edge_set if u not in output_layer_nodes and v not in output_layer_nodes}
            curvature_wo_output = sum(edge_curvatures.get((u, v), 0.0) for (u, v) in edges_wo_output)
    
            external_in_degree = sum(
                1 for (u, v) in threshold_edges if v in community_nodes and u not in community_nodes
            )

            communities[community_id] = {
                "node_count": total_nodes_all,
                "edge_count": total_edges_all,
                "nodes": list(community_nodes),
                "edges": list(community_edge_set),
                "total_internal_curvature": curvature_all,
                ...
                "stats_wo_output_layer": {
                    "node_count": len(nodes_wo_output),
                    "edge_count": len(edges_wo_output),
                    "total_internal_curvature": curvature_wo_output
                }
            }

            community_id += 1

    # === Layer-wise in-degree ===
    in_degrees_per_layer = defaultdict(list)
    for node_id in in_degree_negative:
        layer = np.searchsorted(prefix_dims, node_id, side='right') - 1
        in_degrees_per_layer[layer].append(in_degree_negative[node_id])
    ...
    for cid, data in communities.items():
        ...
        node_indegree_map = {
            node: in_degree_filtered[node]
            for node in nodes if node in in_degree_filtered
        }
    ...
    return {
        ...
        "communities_w_input_layer": communities,
