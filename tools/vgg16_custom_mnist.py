...
class VGG16_CIFAR10(nn.Module):
    def __init__(self, model_info, edge_set, device, input_c = 1, num_classes = 10, start_l=1):
        super().__init__()
        ...
        self.features = nn.Sequential(
            *conv_block(input_c, 64, 2),     # → 64 × 14 × 14
            *conv_block(64, 128, 2),         # → 128 × 7 × 7
            *conv_block(128, 256, 3),        # → 256 × 3 × 3
            *conv_block(256, 512, 2),        # → 512 × 1 × 1 (STOP HERE)
        )

        # Final conv layer (no pool after this)
    ...
    def forward(self, x):
        ...
        # fc
        fc = x_cov1.view(-1, self.num_flat_features(x_cov1))
    ...
    def NN_info_batch(self, x):
        ...
        x_tmp = x_cov1
        ones_tmp = torch.ones_like(x_cov1)
        
        nodes = torch.cat((nodes, x_cov1.view(-1, self.num_flat_features(x_cov1))), axis = 1)

        # fully connected
        x = x_cov1.view(-1, self.num_flat_features(x_cov1)) # batch * input size
        x_tmp = x
        ones_tmp = torch.ones_like(x)
