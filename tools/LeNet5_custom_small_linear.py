...
class LeNet_custom_v2(nn.Module):
    ...
    def forward(self, x):
        ...
        # first CNN
        x_cov1 = self.CNN(x, self.conv1.weight, self.conv1.bias.unsqueeze(1), 1, 2)
        
        # # first pooloing
        # x_pool1 = self.maxpooling(x_cov1, 2, 3)
        
        # second CNN
        x_cov2 = self.CNN(x_cov1, self.conv2.weight, self.conv2.bias.unsqueeze(1), 2, 3)
        ...
        fc1 = self.linear(fc, self.fc1, 3, 4)
        
        fc2 = self.linear(fc1, self.fc2, 4, 5)
