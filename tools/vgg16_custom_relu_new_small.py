...
class VGG16_CIFAR10_small(nn.Module):
    def __init__(self, model_info, edge_set, device, prefix_dims = [], input_c = 3, num_classes = 10, start_l=1):
        ...
        self.conv3_1 = nn.Conv2d(128, 128, kernel_size=3, padding=0)  # 18
        self.bn3_1 = nn.BatchNorm2d(128)
        
        self.conv3_2 = nn.Conv2d(128, 128, kernel_size=3, padding=0)  # 16
        self.bn3_2 = nn.BatchNorm2d(128)

        self.conv3_3 = nn.Conv2d(128, 128, kernel_size=3, padding=0)  # 14
        self.bn3_3 = nn.BatchNorm2d(128)
        
        self.conv4_1 = nn.Conv2d(128, 128, kernel_size=3, padding=0)  # 12
    ...
    def forward(self, x):
        ...
        # forth CNN
        x_cov11 = self.activation((self.CNN(x_cov10, self.conv5_1.weight, self.conv5_1.bias.unsqueeze(1), 11,12)))
        x_cov12 = self.activation((self.CNN(x_cov11, self.conv5_2.weight, self.conv5_2.bias.unsqueeze(1), 12,13)))
        x_cov13 = self.activation((self.CNN(x_cov12, self.conv5_3.weight, self.conv5_3.bias.unsqueeze(1), 13,14)))
        
        # fc
        fc = x_cov13.view(-1, self.num_flat_features(x_cov13))
        
        fc1 = self.activation(self.linear(fc, self.fc1, 14,15))
        
        fc2 = self.activation(self.linear(fc1, self.fc2, 15,16))
    ...
    def NN_info_batch(self, x):
        ...
        x = self.activation((self.CNN(x, k1, b1.unsqueeze(1),i,j)))
