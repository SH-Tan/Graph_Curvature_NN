...
from .remove_edge_cifar_union_iter import *
...
def iterative_edge_removal_and_curvature(args):
    seed = 13
    ...
    val_set = load_dataset_from_disk("./data/CIFAR10_val", batch_size=64, shuffle=True)
    sep_dataloader = utils.sep_label(val_set, selected_classes, bs=16)
    ...
    prev_removed_edges = set()
    prefix = f"{model_full_n}_{metric}_{dataset}_batch"
    
    for iter_id in range(1, total_iterations + 1):
        print(f"\n=== Iteration {iter_id} / {total_iterations} ===")

        # Step 1: reload full model and apply previously removed edges
        net_iter = copy.deepcopy(net_H)
        if len(prev_removed_edges) > 0:
            print(f"Applying {len(prev_removed_edges)} previously removed edges...")
            net_iter.__build_remove_mask__(list(prev_removed_edges), len(prev_removed_edges))
            
        # Step 2: curvature collection
        ...
        prefix = f"{model_name}_{metric}_{dataset}_iter{iter_id}_batch"
        
        # search for any file starting with that prefix
        matches = glob.glob(os.path.join(res_path, f"{prefix}*"))
        ...
        if matches:
            ...
            pos_edges_only = remove_edge_cifar_union(args, net_full, prefix = prefix, prev_removed_edges = prev_removed_edges, iter_id = iter_id)

            prev_removed_edges |= set(pos_edges_only)
            continue
        ...
        while len(finished_labels) < len(selected_classes):
                        ...
                        for idx in range(images.shape[0]):
                            ...
                            with torch.no_grad():
                                ...
                                if metric.lower() == "w1":
                                    ...
                                    weights_inv1, weights_inv2 = net_iter.normalization_weight_w3(nodes_ori, weights, dims, model_dims_small)
                                    weights_inv = weights_inv1.detach()
                                    weights_inv2 = weights_inv2.detach()
                                    ...
                                    weights_inv1, weights_inv2, weights_inv3 = net_iter.normalization_weight_w4(nodes_ori, weights, dims, model_dims_small)
                                    weights_inv = weights_inv1.detach()
                                    weights_inv2 = weights_inv2.detach()
            ...
            if all(len(res_l[l]) == 10 for l in selected_classes if label_progress[l] < sample_size) or finished_l >= len(selected_classes):
                save_name = f"{model_name}_{metric}_{dataset}_iter{iter_id}_batch{round_id}.pkl"
        ...
        pos_edges_only = remove_edge_cifar_union(args, net_full, prefix = prefix, prev_removed_edges = prev_removed_edges, iter_id = iter_id, device=device)
        
        if (len(pos_edges_only) < 1):
            break
