...
def remove_edge_cnn_per_allL(args):
    seed = 29
        ...
        with open(res_path + "edge_cnn_tanh_" + str(l) + ".txt", "w+") as ff:
            ...
            for (images, labels) in sep_dataloader[l]:
                for idx in range(images.shape[0]):
                    ...
                    ricci_curvature, sp_dict = graph_curvature_main_torch(dims, weights_inv, device=device, probability_w=weights_inv2, alpha=alpha, model_dims=model_dims)

                    neg_e, pos_e = get_top_c(ricci_curvature, 1, prefix_dims, threshold = -50)
