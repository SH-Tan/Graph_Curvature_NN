...
def cal_edge_v(net, im, device):     
    ...
    w_avg = np.mean(edge_array, axis=0)
...
# change -10000 to 0 
def q_adjust(adj, nodes, q_exp, model_dims):
    # input layer nodes num
    cur_dim = model_dims[1]["dim"]
    cur_size = cur_dim['out_size']
    cur_nodes = cur_dim['channel'] * cur_size**2
      
    node_v = nodes[0].cpu()
    ind = torch.where(node_v <= 0)[0]
    # ind = np.where(adj == 0)[0]
    new_adj = np.zeros_like(adj)
    adj[:, ind[ind >= cur_nodes]] *= -1 
...
if __name__ == '__main__':
    ...
    for q in Q:
        ...
        with open(res_path + "q=" + str(q) + "_6_linear.txt", "w+") as f:
            for e in eps:
                f.write(f'Q = {q}, eps = {e}....\n\n')
                ...
                for l in selected_classes:
                    ...
                    for (ori_im, adv_im) in succ_pair[l]:
                        ...
                        for im in ori_im:
                            ...
                            # adj = q_adjust(adj_m_ori, nodes_ori, q_exp, model_dims)
                ...
                with open(res_path + str(e) + "_w6_" + str(q) + '_' + "frac_robust_linear.pkl", 'wb') as file:
                    pickle.dump(rob_fraction, file)
                with open(res_path + str(e) + "_w6_" + str(q) + '_' + "frac_norobust_linear.pkl", 'wb') as file:
                    pickle.dump(non_fraction, file)
                    
                with open(res_path + "_w6_" + str(q) + "_ori_c_" + str(e) + "_robust_linear.pkl", 'wb') as file:
                    pickle.dump(robust_c, file)
                with open(res_path + "_w6_" + str(q) + "_ori_c_" + str(e) + "_norobust_linear.pkl", 'wb') as file:
