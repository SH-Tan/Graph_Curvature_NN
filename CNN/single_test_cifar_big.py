...
def cifar_big_main(args):
    ...
    sep_dataloader = utils.sep_label(test_dataset, selected_classes, bs=1000)
    ...
    net_H = CNN_custom(model_dims, device, input_c=3)
    ...
    for q in Q:
        with open(res_path + "cifar10_big.txt", "w+") as f:
            f.write(f'dims = {dims}\n\n')
            for e in eps:
                f.write(f'eps = {e}....\n\n')
                ...
                with open(res_path + model_full_n + str(e) + metric + str(q) + "frac_robust_cifar_big.pkl", 'wb') as file:
                    pickle.dump(rob_fraction, file)
                with open(res_path + model_full_n + str(e) + metric + str(q) +  "frac_norobust_cifar_big.pkl", 'wb') as file:
                    pickle.dump(non_fraction, file)
                    
                with open(res_path + model_full_n + str(e) + metric + str(q) + "curv_robust_cifar_big.pkl", 'wb') as file:
                    pickle.dump(robust_c, file)
                with open(res_path + model_full_n + str(e) + metric + str(q) + "curv_norobust_cifar_big.pkl", 'wb') as file:
