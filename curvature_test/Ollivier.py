...
def _distribute_densities(source, target):
    ...
    print(f'source is {source}, target is {target}, source_topknbr = {source_topknbr}, target_topknbr = {target_topknbr}... x = {x}, y = {y}, d = {d}...\n')
    return x, y, d
...
def _compute_ricci_curvature_edges(G: nx.Graph, weight="weight", DG = None, edge_list=[],
    ...
    _Gk = nk.nxadapter.nx2nk(DG, weightAttr=weight)
    _alpha = alpha
    _weight = weight
    _method = method
    _base = base
...
def _compute_ricci_curvature(G: nx.Graph, weight="weight", DG = None, **kwargs):
    ...
    **kwargs
    ...
    edge_ricci = _compute_ricci_curvature_edges(G, weight=weight, DG= DG, **kwargs)
...
class OllivierRicci:
    ...
    def __init__(self, G: nx.Graph, weight="weight", alpha=0.5, method="OTDSinkhornMix",
        ...
        """
        self.G = G.copy()
        self.DG = G.copy() if DG == None else DG.copy()
        self.alpha = alpha
        self.weight = weight
        self.method = method
        self.base = base
    ...
    def compute_ricci_curvature(self):
        ...
        self.G = _compute_ricci_curvature(G=self.G, weight=self.weight, DG = self.DG,
                                          alpha=self.alpha, method=self.method,
                                          base=self.base, exp_power=self.exp_power,
