...
def normalization_weight_w1(nodes, weights, dims):
    # batch = nodes.shape[0]
...
def normalization_weight_w2(nodes, weights, dims):
    # batch = nodes.shape[0]
    nodes_num = nodes.shape[1]
    prefix_dims = torch.cumsum(torch.tensor(dims), dim=0)
    prefix_dims = torch.cat([torch.tensor([0]), prefix_dims])
...
def normalization_weight_w6(nodes, weights, dims, q):
    # batch = nodes.shape[0]
    nodes_num = nodes.shape[1]
    prefix_dims = torch.cumsum(torch.tensor(dims), dim=0)
    prefix_dims = torch.cat([torch.tensor([0]), prefix_dims])
...
# weights regularization 
def normalization_weight_w3(nodes, weights, dims):
    # batch = nodes.shape[0]
    nodes_num = nodes.shape[1]
    prefix_dims = torch.cumsum(torch.tensor(dims), dim=0)
    prefix_dims = torch.cat([torch.tensor([0]), prefix_dims])
