...
def getNN_info(model, input):
    ...
    ones = torch.ones_like(curNeurons).to(device)
    ...
    for layer in range(layerCount-1):
        ...
        ones = weights[layer+1]@ones + offsets[layer+1].reshape(len(offsets[layer+1]),1)
        ...
        # x = torch.reshape(weights[layer+1]*curNeurons, (1, cur_shape))
        # print(x_tmp)
        ...
        ones_tmp = torch.ones_like(ones.T)

    return dims, nodes_num, edges_num, NN_w, edge_v, nodes, nodes_ori
