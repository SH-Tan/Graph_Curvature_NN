...
def _distribute_densities(source, target):
    ...
    # ori_w = _dirG[source][target]['weight'] 
    # _dirG[source][target]['weight'] = 1e10
    # pairs = np.ix_(source_topknbr, target_topknbr)
    
    # d = np.zeros((pairs[0].shape[0], pairs[1].shape[1]), dtype=np.float32)
    
    # dis = _Gk.weight(source, target)
    
    # for i, s in enumerate(pairs[0]):
    #     # dis += _Gk.weight(s[0], source) * x[i]
        
    #     for j, t in enumerate(pairs[1][0]):
    #         # dis += _Gk.weight(target, t) * y[j]
            
    #         d1 = nx.shortest_path_length(_dirG, source=s[0], target=t, weight='weight')
    #         if (d1 >= 1e10):
    #             d1 = d1 - 1e10 + ori_w
    #             # print(d1)
    #         d[i][j] = d1
  
    # gk = nk.nxadapter.nx2nk(_dirG, weightAttr='weight')
    
    # apsp = nk.distance.APSP(gk).run().getDistances()
    # apsp = np.array(apsp)
    # _dirG[source][target]['weight'] = ori_w
    
    d = _apsp[np.ix_(source_topknbr, target_topknbr)]  # transportation matrix
    # print(f'x = {x}, y = {y} \n')
    
    dis = _Gk.weight(source, target)
    if (len(source_topknbr) > 1):
        for i, src in enumerate(source_topknbr):
            dis += (_Gk.weight(src, source) * x[i])/2
    
    if (len(target_topknbr) > 1):
        for i, tgt in enumerate(target_topknbr):
            dis += (_Gk.weight(target, tgt) * y[i])/2
    ...
    #     dis = _Gk.weight(source, target)
    #     for i, src in enumerate(source_topknbr):
    #         dis += _Gk.weight(src, source) * x[i]
            
    #     for tgt in target_topknbr:
    #         dis += _Gk.weight(target, tgt) * y[i]
                
        # d = _apsp[np.ix_(source_topknbr, target_topknbr)]  # transportation matrix
    
    # print(f'for {source} to {target}, x is {x}, source nbr is {source_topknbr}, y is {y}, target nbr is {target_topknbr}, shorest path is {d}\n')
...
def _compute_ricci_curvature_single_edge(source, target):
    ...
    if _method == "OTD":
        x, y, d, dis = _distribute_densities(source, target)
    ...
    # print(f'the weight is {_Gk.weight(source, target)}, dis = {dis} \n\n')
    
    # if (len(x) == 1 and len(y) == 1):
    #     print(f'x = {x}, y = {y}, d1 = {d}, dis = {dis}, m = {m}, W = {result}\n')
...
class OllivierRicci:
    ...
    def recal_qexp(self, q, node_v, weight="weight"):
