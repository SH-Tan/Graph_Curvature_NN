...
class OllivierRicci:
    ...
    def recal_graph_weight_tanh(self, nodes, weight="weight"):
        Gk = nk.nxadapter.nx2nk(self.G, weightAttr=weight)
        ...
        for n in list(self.G.nodes()):
            ...
            if sum > 0:  
                for nbr in neighbors:
                    if self.G[nbr][n]['weight']*nodes[0][nbr].cpu().item() < 0:
                        self.G.remove_edge(nbr, n)
                    else:
                        self.G[nbr][n]['weight'] = 1./(self.G[nbr][n]['weight'] * (sum/pos_sum) * nodes[0][nbr].cpu().item())
                ...
                for nbr in neighbors:
                    if self.G[nbr][n]['weight']*nodes[0][nbr].cpu().item() > 0:
                        ...
                        self.G[nbr][n]['weight'] = 1./(-self.G[nbr][n]['weight'] * (sum/negative_sum) * nodes[0][nbr].cpu().item())
